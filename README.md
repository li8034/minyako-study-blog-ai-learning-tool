...
`sdxx/blog/` 是一份可直接上传、可继续二次配置的公开版本模板。它保留产品名称与完整核心功能，但不包含任何现成 `pages` 内容，也不包含个人身份信息、本地仓库信息或私有 API Key，适合作为初始化工程、公开仓库底稿或交付版本。

## 项目优势

- 支持直接导入 Markdown 与 PDF，并自动生成独立文章目录。
...
- 可以公开提交前端、后端、空白 `content/` 模板与说明文档。
- 不要提交 `content/settings.local.json`、个人密钥、本机缓存和个人文章数据。
- 当前版本已经移除了预置文章，适合直接作为公开仓库初始版本。
